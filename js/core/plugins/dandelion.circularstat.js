	...
	daCircularStat.prototype = {
		...
		_getVal: function(progress) {
			if(this.config.percent)
				...
				v = progress * (this.data.endValue - this.data.startValue);
				return progress > 1.0? this.data.endValue : v.toFixed(this.config.decimals);
			}
		}, 
		...
		_update: function(timeout, init) {
			...
			if(init) {
				...
				this.valueEl.innerHTML = 
					this.config.percent? 
						('<span>' + val + '%</span>') : 
						('<span>' + val + '</span>' + '/' + this.config.maxValue.toFixed(this.config.decimals));
			}
		}
	}
