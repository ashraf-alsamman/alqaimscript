/*
 * Dandelion Admin v1.2 - Wizard Form JS
 *
...
;(function( $, window, document, undefined ) {
	...
	daWizard.prototype = {
		...
		initSteps: function() {
			...
			this.wizardNav.find('.da-wizard-progress')
				.animate({
					width: (100 / this.steps.size()).toFixed(2) + "%"
				}, 'slow')
			.end()
				.find('ul').children('li').removeClass('active done').first().addClass('active');
			...
			if(stepNumber !== this.data.activeStep) {
				...
				this.steps.eq(this.data.activeStep).data('done', true).fadeOut('fast', function() {
					...
					self.wizardNav.find('.da-wizard-progress')
						.animate({
							width: ((100 / self.steps.size()) * (stepNumber + 1)).toFixed(2) + "%"
						}, 'slow')
					.end()
						.find('ul')
		...
		resetWizard: function() {
			this.steps.each(function() { $(this).data('done', null); });
		...
		submitForm: function() {
			...
			if(false !== shouldSubmit) {
				if(this.config.ajaxSubmit && $.fn.ajaxSubmit) {
					this.$elem.ajaxSubmit(this.config.ajaxOptions);
				} else
					this.$elem.submit();
			}
		}, 
		...
		_fixWizardProgress: function() {
			var wzdNav = this.wizardNav, 
				maxProgressWidth = 0;
				
			$('ul', wzdNav)
				.children('li').each(function() { maxProgressWidth += $(this).width(); });
			wzdNav
				.find('.da-wizard-progress').first().css('maxWidth', maxProgressWidth);
		}, 
		...
		_buildButtons: function() {
			var btnContainer = $(this.config.buttonContainer, this.$elem), 
				...
				$submitButton = $button.clone().val(this.config.submitButtonLabel).attr("name", this.config.submitButtonName).addClass(this.config.submitButtonClass).bind('click', function(event) {
					self.submitForm();
					event.preventDefault();
				});
	...
	$.fn.daWizard = function(options) {
		return this.each(function() {
			if (undefined == $(this).data('daWizard')) {
                var instance = new daWizard(this, options).init();
                $(this).data('daWizard', instance);
            }
		});
	};
