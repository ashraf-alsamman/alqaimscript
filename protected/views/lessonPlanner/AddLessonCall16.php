...
<textarea  class="note"  cols="10" rows="1"  placeholder="Lesson note"></textarea>
<?php
...
'live' =>'false'),array('id' => 'send-link-'.uniqid(),'class'=>'AddLessondata16')); 
 ?><br><?php
 
 $NewLessonPlannerLesson = new LessonPlannerLesson;
$this->widget('ext.uploadify.MUploadify',array(
  'name'=>"LessonPlannerLesson_name16",
  'scriptData' => array(
...
'model'=>$NewLessonPlannerLesson,
     //   'debug' => true,
'attribute'=>'upload_ex',
'script'=>$this->createUrl('UploadLesson16'),
'fileExt' => '*.docx;*.doc;',
'fileDesc' =>  '*.docx;*.doc;',
'buttonText'=>('Save Lesson'),
'auto'=>true,
'onAllComplete'=>'js:function(event, ID, fileObj, response, data){
$("#all_lessons16").load(\''.$this->createUrl("GetAlllessonsByWeek", array(
)
).'\'); 
   $(".AddLessondata16").click();
  
  $("#AddLessonCall16").load(\''.$this->createUrl("AfterLessonDone", array(
)
).'\'); 
