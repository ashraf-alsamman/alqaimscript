...
<textarea  class="note"  cols="50" rows="2"  placeholder="Lesson note"></textarea>
<?php
...
'live' =>'false'),array('id' => 'send-link-'.uniqid(),'class'=>'AddLessondata13')); 
 ?><br><?php
 
 $NewLessonPlannerLesson = new LessonPlannerLesson;
$this->widget('ext.uploadify.MUploadify',array(
  'name'=>"LessonPlannerLesson_name13",
  'scriptData' => array(
...
'model'=>$NewLessonPlannerLesson,
     //   'debug' => true,
'attribute'=>'upload_ex',
'script'=>$this->createUrl('UploadLesson13'),
'fileExt' => '*.docx;*.doc;',
'fileDesc' =>  '*.docx;*.doc;',
'buttonText'=>('Save Lesson'),
'auto'=>true,
'onAllComplete'=>'js:function(event, ID, fileObj, response, data){
$("#all_lessons13").load(\''.$this->createUrl("GetAlllessonsByWeek", array(
)
).'\'); 
   $(".AddLessondata13").click();
  
  $("#AddLessonCall13").load(\''.$this->createUrl("AfterLessonDone", array(
)
).'\'); 
