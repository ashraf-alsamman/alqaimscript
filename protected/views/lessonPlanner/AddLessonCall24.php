...
'live' =>'false'),array('id' => 'send-link-'.uniqid(),'class'=>'AddLessondata24')); 
 ?><br><?php
 
 $NewLessonPlannerLesson = new LessonPlannerLesson;
$this->widget('ext.uploadify.MUploadify',array(
  'name'=>"LessonPlannerLesson_name24",
  'scriptData' => array(
...
'model'=>$NewLessonPlannerLesson,
     //   'debug' => true,
'attribute'=>'upload_ex',
'script'=>$this->createUrl('UploadLesson24'),
'fileExt' => '*.docx;*.doc;',
'fileDesc' =>  '*.docx;*.doc;',
'buttonText'=>('Save Lesson'),
'auto'=>true,
'onAllComplete'=>'js:function(event, ID, fileObj, response, data){
$("#all_lessons24").load(\''.$this->createUrl("GetAlllessonsByWeek", array(
)
).'\'); 
   $(".AddLessondata24").click();
  
  $("#AddLessonCall24").load(\''.$this->createUrl("AfterLessonDone", array(
)
).'\'); 
