...
'live' =>'false'),array('id' => 'send-link-'.uniqid(),'class'=>'AddLessondata10')); 
 ?><br><?php
 
 $NewLessonPlannerLesson = new LessonPlannerLesson;
$this->widget('ext.uploadify.MUploadify',array(
  'name'=>"LessonPlannerLesson_name10",
  'scriptData' => array(
...
'model'=>$NewLessonPlannerLesson,
     //   'debug' => true,
'attribute'=>'upload_ex',
'script'=>$this->createUrl('UploadLesson10'),
'fileExt' => '*.docx;*.doc;',
'fileDesc' =>  '*.docx;*.doc;',
'buttonText'=>('Save Lesson'),
'auto'=>true,
'onAllComplete'=>'js:function(event, ID, fileObj, response, data){
$("#all_lessons10").load(\''.$this->createUrl("GetAlllessonsByWeek", array(
)
).'\'); 
   $(".AddLessondata10").click();
  
  $("#AddLessonCall10").load(\''.$this->createUrl("AfterLessonDone", array(
)
).'\'); 
