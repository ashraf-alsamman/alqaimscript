...
switch($itemCssClass)
{
	case 'msg-read': $status = ($this->getAction()->getId()=='sent')? Yii::t($language,'Recipient has read your message') : Yii::t($language,'Message has been read') ; break;
	case 'msg-deliver':  $status = Yii::t($language,'Recipient has not read your message yet');
	case 'msg-new': $status =  ($this->getAction()->getId()=='sent')? Yii::t($language,'Recipient has not read your message yet') : Yii::t($language,'You received a new message'); break;
	case 'msg-sent': $status = Yii::t($language,"You sent message").$username.Yii::t($language,"a message");
}
...
<tr class="mailbox-item <?php echo $itemCssClass; ?> <?php if($this->getAction()->getId()!='sent') echo 'mailbox-draggable-row'; ?>">
    ...
    <td width="150" class="mailbox-received">
		<div align="right" class="mailbox-item-wrapper" style="width:80px">
			...
			<div class="mailbox-replied" title="<?php echo Yii::t($language,'this message has been replied to') ?>">&nbsp;&nbsp;</div>
			<?php endif; ?>
			<?php echo $received; ?>
		</div>
