...
foreach($conv->messages as $msg): 
	$sender = $this->module->getUserName();
	
	if ($msg->recipient_id!=Yii::app()->user->id){$this->redirect(array('../404'));}
	if(!$sender)
...
<?php	echo ($msg->sender_id == Yii::app()->user->id)? Yii::t($language,'You') : ucfirst($sender);
...
$form=$this->beginWidget('CActiveForm', array(
	...
	<?php echo $form->error($reply,'text'); ?>
		<div class="mailbox-textarea-wrap ui-helper-clearfix">
			<textarea name="text" cols="50" rows="7" placeholder="<?php echo Yii::t($language,'Reply here...') ?>"></textarea>
		</div>
	<input type="submit" class="btn btn-large mailbox-input" value="<?php echo Yii::t($language,'Send Reply') ?>" />
	</div>
